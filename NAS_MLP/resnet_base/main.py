...
short_cut_num_record=np.ones(net_depth) #the number of layer's connection
short_cut_record=np.zeros([net_depth,net_depth])# the index between some layer with former layer
...
class SimpleNet(nn.Module):
    def __init__(self):
        ...
        self.features.add_module("net_name[0]", nn.Linear(2, net_arch[0]))
        for i in range(net_depth-2):
            self.features.add_module(net_name[i+1],nn.Linear(net_arch[i], net_arch[i+1]))  
        self.features.add_module(net_name[net_depth-1],nn.Linear(net_arch[net_depth-1], 2))      
#        self.layer_dict={}
...
#        self.layer_dict[net_name[net_depth-1]]=nn.Linear(net_arch[net_depth-1], 2)
    ...
    def forward(self, x):
        ...
        for layer_idx in range(net_depth-2):
            layer_name='fc'+str(layer_idx+2)
            out_dict[str(layer_idx+2)]=self.layer_dict[layer_name](out_dict[str(layer_idx+1)])
            for k in range(short_cut_num_record[layer_idx]):
                link_name='l'+str(short_cut_record[layer_idx,0])+'_'+str(layer_idx+2)
                out_dict[str(layer_idx+2)]=out_dict[str(layer_idx+2)]+out_dict[str(short_cut_record[layer_idx,0])]*self.link_dict[link_name]
            out_dict[str(layer_idx+2)]=F.relu(out_dict[str(layer_idx+3)])
