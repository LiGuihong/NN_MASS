...
import datetime
#-------------------------------------
file_logs=open('logs5_batch.txt','a+')
train_logs=open('train5_batch.txt','a+')
features_logs=open('feat5_batch.txt','a+')
# argument parser
...
def data_generation():
    ...
    np.savetxt("train_v5.csv", np.array(train_data), delimiter=',')
    np.savetxt("test_v5.csv", np.array(test_data), delimiter=',')
#data_generation()
...
class SimpleNet(torch.nn.Module):
    def __init__(self):
        ...
        batchnorm_list.append(nn.BatchNorm1d(net_arch[0]+layer_cut[0]))
        for i in range(net_depth-2):
            layer_list.append(nn.Linear(net_arch[i+1]+layer_cut[i+1], net_arch[i+1]))
            batchnorm_list.append(nn.BatchNorm1d(net_arch[i+1]+layer_cut[i+1]))  
...
#        self.layer_dict[net_name[net_depth-1]]=nn.Linear(net_arch[net_depth-1], 2)
    ...
    def forward(self, x):
        ...
        out0=F.relu(self.batchnorm[0](self.features[0](x)))        
        out1=self.features[1](F.relu(self.batchnorm[1](out0)))
        out_dict={}
        out_dict['0']=out0
        out_dict['1']=out1
        for layer_idx in range(net_depth-2):
            ...
            if layer_idx<net_depth-3:
                #out_dict[str(layer_idx+2)]=F.relu(self.batchnorm[layer_idx+2](out_tmp))
                out_tmp=F.relu(self.batchnorm[layer_idx+2](out_tmp))
            #else:
            #    out_dict[str(layer_idx+2)]=out_tmp
            #out_tmp=self.features[layer_idx+2](out_tmp)
            out_dict[str(layer_idx+2)]=self.features[layer_idx+2](out_tmp)
        #print(out_dict)
...
class SimpleNet_1(torch.nn.Module):
    def __init__(self):
    ...
    def forward(self, x):
        ...
        out1=self.features[1](F.relu(out0))
        out_dict={}
        out_dict['0']=out0
        out_dict['1']=out1
        for layer_idx in range(net_depth-2):
...
class SimpleNet_2(torch.nn.Module):
    def __init__(self):
...
def make_dataset(train_file='train_v1.csv',test_file='test_v1.csv'):
    ...
    np.savetxt("tr_v1.csv", np.array(train_label), delimiter=',')
    for i in range(test_num):
        idx=int(i/(test_num/num_seg))
        test_label[i]=int(idx % 2)
    np.savetxt("te_v1.csv", np.array(test_label), delimiter=',')
...
for iter_times in range(3):
    ...
    #print('------******------******------******------******------******------******------',file=file_logs)
    print(str(nn_mass) +', '+str((100 * float(correct) / total))+', '+str(max_acc)+', '+str(net_width)
        +', '+str(net_depth)+', '+str(density)+', '+str(num_seg)+','+str(bound_num)+','+str(bound),file=file_logs) 
    print(model.features,file=features_logs)
