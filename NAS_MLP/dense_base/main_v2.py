...
import datetime  
#-------------------------------------
file_logs=open('logs2_.txt','a+')
train_logs=open('train2_.txt','a+')
# argument parser
...
parser.add_argument('--epochs', type=int, default=2000, help='Number of epoch to train')
...
class SimpleNet(torch.nn.Module):
    def __init__(self):
        ...
        for i in range(net_depth-2):
            layer_list.append(nn.Linear(net_arch[i]+layer_cut[i], net_arch[i+1]))  
        layer_list.append(nn.Linear(net_arch[net_depth-2]+layer_cut[net_depth-2], 2))     
...
for iter_times in range(5):
    ...
    for epoch in range(num_epochs):
        for x in enumerate(train_loader):
            images = x[1]['image'].float().to(device)
            labels = x[1]['landmarks']
