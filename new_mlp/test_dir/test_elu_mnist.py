...
import collections
#-------------------------------------
file_logs=open('logs/log_elu_mnist_v0.txt','a+')
train_logs=open('logs/train_elu_mnist_v0.txt','a+')
# argument parser
...
def test(epoch,test_loader,model,input_size=[batch_size,28*28]):
    ...
    #    (100 * accuracy))
    return accuracy,avg_loss,bound_num,bound
#---------------------for test----------------------
'''
model = SimpleNet_order_no_batch()
...
#            acc_test.append(float(correct)/float(test_num))
    #print('------******------******------******------******------******------******------',file=train_logs)
    # Test the Model
    tmp_acc,tmp_loss,bound_num,bound=test(0,test_loader=test_loader,model=model)
