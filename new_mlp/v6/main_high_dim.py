...
#-------------------------------------
file_logs=open('logs/log_high_dim_v0.txt','a+')
train_logs=open('logs/train_high_dim_v0.txt','a+')
# argument parser
...
parser.add_argument('--num_seg', type=int, default=20, help='Learning rate')
...
parser.add_argument('--data_dim', type=int, default=32, help='the number of tc')
...
class SimpleNet_order_no_batch(torch.nn.Module):
    def __init__(self):
        ...
        batchnorm_list=[]
        layer_list=[]
        layer_list.append(nn.Linear(data_dim, net_arch[0]+layer_tc[0]))
...
def make_dataset(train_file='train.csv',test_file='test.csv'):
    train_data_raw = np.loadtxt(open(train_file,"rb"), delimiter=",",dtype=float)
    test_data_raw = np.loadtxt(open(test_file,"rb"), delimiter=",",dtype=float)
    print(test_data_raw.T.size)
    ...
    np.savetxt("tr_65536.csv", np.array(train_label), delimiter=',')
    for i in range(test_num):
        idx=int(i/(test_num/num_seg))
        test_label[i]=int(idx % 2)
    np.savetxt("te_65536.csv", np.array(test_label), delimiter=',')

    train_tmp=train_data_raw.T[:,0]
    test_tmp=test_data_raw.T[:,0]
    return train_data_raw,test_data_raw,train_label.T,test_label.T

[train_data_raw,test_data,train_label_raw,test_label]=make_dataset(train_file='data/train_32.csv',test_file='data/test_32.csv')
...
for iter_times in range(5):
    ...
    test_data=test_data.view([steps,batch_size,data_dim])
    test_label=test_label.view([steps,batch_size])
