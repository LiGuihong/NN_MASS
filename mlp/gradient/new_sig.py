...
def simulate_m(w,m):
    #normal_mat=np.random.normal(size=(w+int(m/2),w))
...
def make_dataset(train_file='data/train_v0.csv',test_file='data/test_v0.csv'):
...
for i in range(width_num): #width 8,16,32,64,128
    ...
    for j in range(depth_num): #depth  16,20,24
        ...
        for k in range(tc_num):  #tc 0 2 4 6 8 10 12 
            ...
            sim_nn_mass[j,k]=sim_nn_mass[j,k]/5

    fig_name='fig/' + 'width_'+str(pow(2,i+3))+'_sigma_nnmass_'+str(trial_index)
    ...
    plt.title('Width='+str(pow(2,i+3))+': '+'$\mathbb{E}[\sigma_i]$ vs. $H$    ($R^2$='+str(round(score, 2))+')',fontsize=font_size+2)
    #plt.title('Width='+str(pow(2,i+3))+' $R^2$='+str(round(score,2)),fontsize=font_size+2)
    plt.plot(X,reg2.predict(X),'-p')
    plt.plot(nn_mass_mat.reshape(-1),sim_nn_mass.reshape(-1),'o',)
    plt.xlabel('Corresponding NN-Mass',fontsize=font_size)
    plt.ylabel('Mean Singular Value $\mathbb{E}[\sigma_i]$',fontsize=font_size)
    plt.grid(linestyle=':')

    plt.legend(['linear regression'],fontsize=font_size)
    plt.savefig(fig_name+'_sim.png',ppi=600,format='png')
    plt.savefig(fig_name+'_sim.svg',ppi=600,format='svg')
    print(666)
